...
| 34 | Sign-in links that last a day, and survive being opened (`feature/step34-longer-signin-links`) | `MAGIC_LINK_TTL_SECONDS` is 24 hours, not 15 minutes; `MAGIC_LINK_SINGLE_USE=false` makes a link work until it expires however often it is opened, and setting it true is the way back with no deploy; `GET /api/sign-in-options` now tells the screens the rules so they cannot state them wrongly | the link in the inbox still works the next morning, and works again if the first press did not take; the email and the sign-in screen say so honestly | **Done, not deployed** |
