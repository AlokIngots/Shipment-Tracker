...
**Steps 44–49 are merged to `dev` and NOT deployed** — Alok deploys them.
**Step 50 (shipping details) is built on `feature/shipping-details`**, waiting
to merge; it carries **migration 0015**. The live server's schema is
**0012**; the next deploy runs **0013 and 0014** (and **0015** too once
step 50 is merged), and `safe-deploy.sh` backs up first.
