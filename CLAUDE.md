...
**Last worked on: 19 September 2026.** Steps 1–29 and 31–50 are deployed.
**On 19 September 2026 Alok deployed `dev` at `149f417` (steps 44–50)** and
confirmed it ran clean with the schema at **0015**. The deploy output was not
seen here, so the backup folder name is not recorded. Nothing merged is
waiting to deploy except **step 51 (clearer for the customer), merged to
`dev` 19 Sep 2026 (PR #33, `94ba1b0`) and NOT deployed**. It has no
migration, so the schema stays at 0015 and a plain `safe-deploy.sh` run
takes it live. **Step 52 (documents checked by content; accessibility)
and the customer UI polish (approved by Alok) are merged to `dev` too, not
deployed, no migration.** The next deploy takes 51, 52 and the polish
together; the schema stays at 0015.

| Step | What | Merged (all deployed 19 Sep) | Migration |
| ---- | ---- | ------ | --------- |
...
| 49 | Change password on both halves; wrong current passwords count per account | PR #28 `d01fe72`, 19 Sep | none |
| 50 | Shipping details: customer EORI/address/contact, order date and Shipping Bill, ports, voyage, seal, container size, gross weight | PR #30 `a69cabf`, 19 Sep | 0015 `shipping_details` |
...
**Changing your own password** is step 49: a Change password button on both
halves. It needs the current password, so somebody who has only used the
email link asks staff to Set password first.
...
- The one exception is `POST /api/change-password`, which changes nothing
  ...
  nothing.
  `POST /api/logout` writes only which sign-in was ended.
  `test_the_customer_half_of_the_api_has_no_writes` names all five.
...
| 52 | Documents are what they claim; accessibility gaps (`feature/documents-and-accessibility`), from the 16 Sep audit | an upload that is not really a PDF, JPG or PNG, whatever its name, is refused with a sentence saying so, on the screen and in `add_document.py`; a picture named as the wrong kind is stored as what it is. Staff photo tiles get a real alt text | a photo without a caption is "Photo 2 of 5" to a screen reader, not its file name; opening a photo puts keyboard focus on Close and closing it returns focus to the tile; a Skip to content link is the first thing the keyboard reaches. A Mill Test Certificate scanned as a PNG but named .jpg now downloads as the PNG it is | **Merged to `dev` 19 Sep 2026, not deployed. No migration** |
| 51 | Clearer for the customer (`feature/customer-clarity`), from the 16 Sep audit | the staff hints say "Partly shipped" as the customer now sees it; the staff documents page uses the same list of four expected documents, moved beside the model | a contact line on every screen, sign-in included: "Questions? Write to exports@alokindia.com" (Alok's choice, in `frontend/src/lib/contact.js`); after asking for a sign-in link, "Still nothing? Write to …"; the order status pill reads **Partly shipped** (the stored value is still "Part shipped") with "Some of this order has shipped. More is still to come." under it; the last lot says "final shipment, nothing more to follow on this order"; each shipment lists the documents **still to come** (none on a cancelled shipment) | **Merged to `dev` 19 Sep 2026, not deployed. No migration** |
| 50 | Shipping details that had nowhere to go (`feature/shipping-details`) | **Customers & logins:** address, EORI number, contact name and contact email on each customer (the contact is not a login). **Order form:** order date and Shipping Bill number. **Shipment form:** port of loading, port of discharge, voyage number, seal number, container size (suggestions offered, any text kept) and gross weight, refused if below the net quantity. All of it through the CSV importer too, where a **blank column leaves the stored value alone**. Every change on the Activity tab | the order page shows the order date, and each shipment shows From / To, departure and arrival, shipping line, vessel, voyage, container number and size, seal number (copyable), B/L and gross weight. **Never** the customer's address, EORI, contact or the Shipping Bill number | **Merged to `dev` 19 Sep 2026, deployed 19 Sep 2026. Migration 0015** |
| 49 | Change your own password (`feature/change-own-password`) | a **Change password** button beside Sign out on the staff screen, opening the same screen customers get; a staff member who does not know their current password is told to ask a colleague to use Set password | a **Change password** button beside Sign out on the orders list and on every order; a customer who does not know their current password (a link-only customer never had one they were told) is told to ask Alok Ingots to set one. On the server, a wrong current password now counts against the same per-account limit as sign-in, and that limit is checked there too | **Merged to `dev` 19 Sep 2026, deployed 19 Sep 2026. No migration** |
| 48 | Sign out that really signs out; guessing capped per account (`feature/signout-and-account-limit`) | Sign out from the staff screens ends that sign-in on the server too. Wrong passwords for one login are capped at 10 per 15 minutes from anywhere | the same: Sign out ends the sign-in on the server, so a copy of it stops working; a customer whose password sign-in is locked by somebody guessing is told to wait or use the email link | **Merged to `dev` 18 Sep 2026, deployed 19 Sep 2026. Migration 0014** |
| 47 | Password rule: 8 characters, a letter and a number (`feature/password-min-8`) | Set password states the rule and Suggest one always meets it | the change-password screen states the rule and says what is still missing as you type | **Merged to `dev` 18 Sep 2026, deployed 19 Sep 2026. No migration** |
| 46 | Password sign-in beside the email link (`feature/password-login`) | **Set password** on every login in Customers & logins, handed over by staff; `scripts.set_password --email` on the server | the sign-in screen offers email + password and still the email link; signed in for 30 days, through closing the browser | **Merged to `dev` 18 Sep 2026, deployed 19 Sep 2026. No migration** |
| 45 | No carrier redirect anywhere (`feature/remove-carrier-redirect-box`) | the staff **Track** panel loses the "Track this container on <carrier> ↗" button, the paragraph about opening the carrier's website, and the boxed B/L / container numbers with Copy. It now holds live tracking only (Enable, or Refresh / Stop and the timeline). The B/L and container numbers stay in the shipment's one-line summary | unchanged: the box was already gone for customers (step 43). The B/L and container numbers stay in the shipment facts with their Copy buttons; a shipment without live tracking shows its facts and no timeline, and nothing links off-site | **Merged to `dev` 17 Sep 2026, deployed 19 Sep 2026. No migration** |
| 44 | Live container tracking from ShipsGo (`feature/shipsgo-tracking`) | inside each shipment's **Track** panel: **Enable tracking (1 credit)**, asked for with a warning and done once per shipment; then the ShipsGo reference, who turned it on and whether a credit was used, **Refresh now (free)**, **Stop tracking**, and the same timeline the customer sees. A timer reads every tracked shipment back every `SHIPSGO_REFRESH_EVERY_HOURS` (6). `scripts/shipsgo_check.py` checks the key read-only | a **Live tracking** panel under the shipment facts: status, from, to, loaded on, expected arrival, transshipment, container, and the container's movements (place · move · date · vessel), with a transshipment marked and both vessels named. "Tracking is updating…" until ShipsGo has news. Drawn from the stored copy; no page view calls ShipsGo | **Merged to `dev` 17 Sep 2026, deployed 19 Sep 2026. Migration 0013** |
...
| 2026-09-19 | **Step 49 — Change your own password, both halves** (`feature/change-own-password`, off `dev` at `1a67993`, restore tag `pre-change-own-password`, **no migration**). Audit item 3: since step 31 nothing led to the change-password screen except a temporary password, so nobody could change their own. A **Change password** button now sits beside Sign out on the customer orders list, the customer order page and the staff screen, and opens the existing `ChangePasswordScreen` (App state `changingPassword`; Cancel returns to where you were, Sign out clears it). The screen tells anyone who does not know their current password who can set one (customers: Alok Ingots; staff: a colleague's Set password, since Set password refuses your own account). Server: `POST /api/change-password` checks `ratelimit.by_account` on the user's email before the password and records a failure on a wrong one, so guesses there and at sign-in share one count of `LOGIN_ACCOUNT_MAX_ATTEMPTS`; the 429 says how long to wait. 5 new tests in `test_change_own_password.py` (database needed, so CI only). Frontend lint (13 warnings, unchanged) and build pass. CI green (374 passed). **Merged to `dev` (PR #28, `d01fe72`); not deployed. Not looked at in a browser** | `bb5589b` |
| 2026-09-19 | **Step 50 — Shipping details that had nowhere to go, both halves** (`feature/shipping-details`, off `dev` at `5639e0b`, restore tag `pre-shipping-details`, **migration 0015**: twelve nullable columns, nothing already stored touched). Customers: `address`, `eori_number`, `contact_name`, `contact_email`. Orders: `order_date`, `shipping_bill_no`. Shipments: `port_of_loading`, `port_of_discharge`, `voyage_no`, `seal_no`, `container_size`, `gross_weight` (in the shipment's unit; `dispatched_qty` is the net). ETD/ETA already existed and are unchanged. One module, `app/services/shipping_details.py`, holds the rules the screens and the importer share: an EORI is tidied (upper case, no spaces or dashes) and must be two letters then 1–15 letters or digits; a contact email must look like one; a gross weight cannot be negative or, when the unit is a weight, below the net. Ports, voyage, seal and container size are free text on purpose. `accounts.create_customer` / `update_customer` take `details=`; `None` (as `manage_users.py` passes) leaves them alone, and every check runs before anything is changed. The importer gains twelve optional columns (`customer_address`, `customer_eori`, `customer_contact_name`, `customer_contact_email`, `order_date`, `shipping_bill_no`, and the six shipment ones) and, **unlike the older columns, a blank one leaves what is stored alone**, so an export without them cannot wipe what staff typed; `docs/import-template.csv` carries them. Audit field lists extended, so each change shows on Activity. Customer side: Order date among the order facts; each shipment's facts now run From, To, dates, line, vessel, voyage, container number and size, seal (copyable), B/L, gross weight. Staff orders list: route, container size and gross weight on the shipment line; order date and SB on the opened order. **Staff only, tested against every customer reply:** address, EORI, contact and Shipping Bill. The contact is not used for notifications yet. 14 new tests in `test_shipping_details.py` (3 need no database). Migration rendered clean both ways (`alembic upgrade 0014:0015 --sql`, `downgrade 0015:0014 --sql`). Frontend lint (13 warnings, unchanged) and build pass. CI green (388 passed). **Merged to `dev` (PR #30, `a69cabf`); not deployed. Not looked at in a browser** | `e20eca3` |
| 2026-09-19 | **Steps 44–50 deployed** by Alok: `dev` at `149f417`, with `COMPOSE_FILES="docker-compose.prod.yml docker-compose.server.yml" ./safe-deploy.sh`. Confirmed by Alok: ran clean, schema **0012 → 0015** (migrations 0013 `shipment_tracking`, 0014 `signed_out_tokens`, 0015 `shipping_details`). The output was not pasted here, so the backup folder, whether `PASSWORD_SIGN_IN=true` was set, whether a ShipsGo key was added and `shipsgo_check` run, and the browser checks (Change password, Bucher's shipment details, Set password for Bucher) are **not confirmed in this file**. Notes only | _this commit_ |
| 2026-09-19 | **Step 51 — Clearer for the customer** (`feature/customer-clarity`, off `dev` at `45f6766`, restore tag `pre-customer-clarity`, **no migration**). Audit items I-c and I-d and two nice-to-haves. (1) A contact line in the footer of every screen, sign-in page included, "Questions? Write to exports@alokindia.com" as a mailto link; the address was chosen by Alok and lives in `frontend/src/lib/contact.js`. (2) The link-sent card adds "Still nothing? Write to … and we will get you in." (3) The order status pill shows **Partly shipped** for the stored "Part shipped" (`statusLabel` in `StatusPill.jsx`; server, API, emails and tests keep "Part shipped"), and the customer's order page adds "Some of this order has shipped. More is still to come." (4) The final lot reads "final shipment, nothing more to follow on this order". (5) `ShipmentOut.documents_to_come` lists the expected documents not attached yet, in the usual order, and is empty for a cancelled shipment; the order page shows "Still to come: …" under the documents once at least one is attached (before that, the existing sentence already names all four). `EXPECTED_DOCUMENTS` moved from the staff documents router to `app/models/document.py`, and the router imports it. Staff hints now say "Partly shipped". 4 new tests in `test_customer_clarity.py` (1 needs no database). Frontend lint (13 warnings, unchanged) and build pass. CI green (392 passed). **Merged to `dev` (PR #33, `94ba1b0`); not deployed. Not looked at in a browser** | `f719a6c` |
| 2026-09-19 | **Customer UI polish** (`feature/customer-ui-polish`, off `dev` at `80feea0`, restore tag `pre-customer-ui-polish`, **no migration, no backend change**). Asked for by the user: a visual polish of the customer-facing screens only, **Admin Console untouched**, no change in what anything does. Every new rule is in `frontend/src/styles/customer.css` under `.page--customer`, which `App.jsx` puts on the page for anybody who is not staff (and on the sign-in screens); staff never get it, and a before/after screenshot of the staff orders screen was byte-identical. Customer header: the real Alok Ingots logo (`frontend/src/assets/alok-logo.png`, from the user's `ALOK_Logo.png`) on white with "Customer Portal"; staff keep the navy bar (`Header customer={…}`). Sign-in: a short "what the portal is for" panel beside the unchanged form (under it on a phone), the one decorative use of red. Orders list: each order its own card (still a table to screen readers, same click). Order page: a shipped-share bar under the three totals (from the same two numbers); in each shipment the documents come first, every expected one listed with a filled navy Download button (icon) or a dashed "Coming soon" row (replacing step 51's "Still to come" line); the shipment facts, titled "Shipment details", in a grid panel; live tracking and photos restyled to match. Loading states get a small spinner (still for reduced motion); visible keyboard focus throughout. Checked with a local mock server and headless Edge at 1440 and 390 px: nothing scrolls sideways, Download still fetches the file, a photo still opens. Lint (13 warnings, unchanged) and build pass. Before/after screenshots reviewed by Alok on a private review page. **Approved by Alok and merged to `dev` 19 Sep 2026 (PR #36, `ebf58bf`), CI green (392 passed); not deployed** | `6867cb3` |
| 2026-09-19 | **Step 52 — Documents are what they claim; accessibility gaps** (`feature/documents-and-accessibility`, off `dev` at `80feea0`, restore tag `pre-documents-and-accessibility`, **no migration**). Audit N4 and the accessibility group. (1) `storage.document_suffix` reads a document's first bytes (`%PDF-`, JPEG `FF D8 FF`, PNG signature). `storage.store_document` replaces `store_upload` for staff document uploads: it writes as before (size limit, empty check), reads the head back, deletes and refuses anything that is none of the three (`NOT_A_DOCUMENT`: "That file is not really a PDF, JPG or PNG, whatever its name says…"), and renames a picture stored under the wrong kind to its real suffix (.jpg and .jpeg count as the same), so `media_type_for` serves it as what it is. `store_file`, used by `add_document.py`, applies the same rule and the script prints the refusal. Photos were already opened and re-saved (step 25/29), so they are untouched. Documents already stored are not re-checked. One older test changed its expectation: a PNG uploaded as `mtc.jpg` is now served as `image/png`, not `image/jpeg`. (2) `PhotoGallery`: alt text, tooltip, dialog label and caption use the caption or "Photo N of M", never the file name; opening a photo focuses Close, closing returns focus to the tile that opened it. Staff photo tiles use the same alt text (their visible caption still shows the file name, which staff find useful). (3) A Skip to content link before the header, visible only on keyboard focus, targeting `<main id="main" tabIndex={-1}>`. 7 new tests in `test_documents_are_what_they_claim.py` (6 need no database). Frontend lint (13 warnings, unchanged) and build pass. CI green (399 passed). Merged after the customer UI polish: `App.jsx` keeps both the skip link and the customer header. **Merged to `dev` 19 Sep 2026 (PR #35); not deployed. No screen reader tried** | `b98f263` |
| 2026-09-19 | **Inside screens polish, both halves** (`feature/inside-screens-polish`, off `dev` at `284ce30`, restore tag `pre-inside-screens-polish`, **no migration, no backend or `.env` change**). Asked for by the user: the staff console in the same look as the customer screens, one styling system, fonts Archivo + Public Sans, red only as a signal. `customer.css` is now three scopes set by `App.jsx`: `.page--brand` on every page (header, cards, buttons, fields, pills, live tracking, photos, typefaces), `.page--customer` (sign-in, orders as cards, order page, unchanged from the approved polish) and `.page--staff` (quick actions, tabs as one white bar with the open tab navy, order rows, shipment rows, document rows, small buttons, forms, guide boxes). The staff navy header is gone: `Header` always shows the logo. Fonts self-hosted from npm (`@fontsource-variable/archivo`, `@fontsource-variable/public-sans`, OFL, upright weights only) so the CSP stays `'self'`; Archivo for headings and figures (tabular), Public Sans for text; the customer's order number moved from monospace to Archivo. The sign-in page's decorative red rule is now navy. One layout bug fixed in passing: on a phone a staff shipment row's four buttons ran off the card (Remove was cut off, before this change too); they now wrap. No JSX changed except `App.jsx` (page classes), `Header.jsx` and `main.jsx` (font imports), so every button, tab, form and list does what it did. Checked with the mock server and headless Edge at 1440 and 390 px over sign-in, orders, order page and all five staff tabs: nothing scrolls sideways. Lint (13 warnings, unchanged) and build pass. **Approved by Alok; merged to `dev` 19 Sep 2026 (PR #38, `ad57591`), CI green (399); not deployed** | `c8ce0d4` |
| 2026-09-19 | **Several documents at once** (`feature/bulk-document-upload`, off `dev` at `ad57591`, restore tag `pre-bulk-document-upload`, **no migration, no backend, login or `.env` change**). Asked for by the user. Documents & photos gets an **Upload several files** button (in the summary bar, and one on every shipment that opens it on that shipment) for a `BulkUpload` panel: choose the shipment (listed with its order and customer; documents belong to a shipment, not an order), drag files in or Choose files (multiple), and each file is listed with a type dropdown (the four expected documents, or Other with a typed name, 60 characters, as the server stores). The type is guessed from the file name by `frontend/src/lib/docTypeGuess.js` (PL/PACKING, CI/INV/INVOICE/COMMERCIAL, BL/BOL/B_L/B-L/LADING, MTC/TC/TEST as a word/MILL TEST); an unguessable name gets no guess and must be chosen. Upload all is held while any file is not PDF/JPG/PNG, over 20 MB, empty, has no type, or shares its type with another file in the list (the second would replace the first); a row says when it will replace a document already on the shipment. Upload all sends each file in turn through the existing `POST /api/staff/shipments/{id}/documents`, so the server checks each exactly as before (step 52's content check included) and one refusal stops nothing else; each row then says Uploaded or why not, a failed row stays to be fixed and sent again, and the shipments reload. The one-at-a-time Upload / Replace / Remove buttons are unchanged. Styled in `customer.css` under `.page--staff`. Checked with the mock server and headless Edge: five sample files queued (four guessed right, one set to Other, Certificate of Origin), all uploaded, at 1440 and 390 px with nothing scrolling sideways. Lint (13 warnings, unchanged) and build pass. The inside screens polish row above is merged to `dev` (PR #38, `ad57591`), not deployed. **Approved by Alok; merged to `dev` 19 Sep 2026 (PR #39, `4a131a2`), CI green (399); not deployed** | `4f02a14` |
| 2026-09-19 | **Step 53 — The status moves itself from live tracking** (`feature/status-from-tracking`, off `dev` at `4a131a2`, restore tag `pre-status-from-tracking`, **no migration**). Asked for by the user; Delivered = discharged at the port of discharge, the user's choice. `live_tracking.advance_status`, called by `refresh()` after each read (the timer, Refresh now, and the first read after Enable), moves the shipment FORWARD only: `LOADED` → Shipped, `SAILING` or `ARRIVED` → In transit, `DISCHARGED` → Delivered (`STATUS_FROM_TRACKING`). Never backwards, never out of Cancelled, never when the shipment's B/L is no longer the tracked one. Each move is a `shipment.updated` event in the same commit, source `live tracking` (new `audit.LIVE_TRACKING`), shown on Change history as "Automatic, from live container tracking" with Status before → after. The customer email follows from the new status exactly as after a staff change (still gated by `NOTIFY_ONLY_EMAILS`); the order status follows from the shipments as always. New optional `AUTO_STATUS_FROM_TRACKING` (default true, in `.env.example`; the server's `.env` needs nothing). Staff Track panel says the status moves by itself. Only works on shipments where staff pressed Enable tracking, and only once ShipsGo is live on the server. One older test changed: after Enable reads the box as sailing, re-saving the shipment as Shipped would be a move backwards, so it now saves In transit. 7 new tests at the end of `test_live_tracking.py` (1 needs no database). Frontend lint (13, unchanged) and build pass. The bulk-upload row above is merged to `dev` (PR #39, `4a131a2`), not deployed. **Waiting for the user's approval; not merged, not deployed** | _this commit_ |
...
- **Password rules are short on purpose** — at least 8 characters,
  including a letter and a number, and not the same character repeated
  (12 characters of anything until 18 Sep 2026, lowered on Alok's decision
  because staff now read passwords out to customers). No rules about
  punctuation or capitals: they mostly teach people to write `Password1!`
  and reuse it. The sentence lives once on each side —
  `PASSWORD_RULE` in `security.py` and in `frontend/src/lib/passwordRule.js`
...
- **Password guessing is limited at `/api/login` and, since step 49, at
  `POST /api/change-password`.** A wrong current password there counts
  against the per-account limit (`LOGIN_ACCOUNT_MAX_ATTEMPTS`) and is
  refused once it is reached; the per-address limits apply to sign-in
  only, because change-password needs a valid token first.
