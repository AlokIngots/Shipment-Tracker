...
content; accessibility), the customer UI polish, the inside-screens polish
(staff console in the same look, Archivo + Public Sans), several documents at
once, and step 53 (status moves itself from live tracking). None had a
migration, so the schema stays at **0015**. Not confirmed here: whether
`SHIPSGO_API_KEY` is set and `scripts.shipsgo_check` passed on the server --
step 53 does nothing until it has. The first deploy that day, of `dev` at
`149f417` (steps 44–50), Alok confirmed ran clean with the schema at 0015.
