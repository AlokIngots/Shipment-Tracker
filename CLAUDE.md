...
**Step 38 — a staff orders list you can scan — is merged to `dev` and
waiting to be deployed.** It is frontend only: no API change, no migration,
and the customer half is untouched. The server is still at step 37 code with
schema **0012**, which does not move. Before deploying, check
...
- **Nothing on the staff orders screen has been clicked by a person.** Step
  38 was proved with a headless browser against a throwaway page holding
  made-up orders, because Docker Desktop will not start on this PC and there
  is no database to run the real screen against. The opening and shutting,
  the keyboard path and the remembered-open behaviour have never met the
  real API.
